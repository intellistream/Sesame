...
public:
    Birch(double threshold, int branching_factor, int n_clusters, int dim)
        : threshold(threshold), branching_factor(branching_factor), n_clusters(n_clusters), dim(dim)
    {
    ...
    py::array_t<int> fit_predict(py::array_t<double> X)
    {
        ...
        for (int i = 0; i < num_vectors; i++)
        {
            SESAME::PointPtr point = std::make_shared<SESAME::Point>(dim, id++, 1.0, 0.0, 0);
            for (int j = 0; j < dim; j++)
            {
                point->data()[j] = ptr[i * dim + j];
            }
    ...
    py::dict get_params()
    {
        return py::dict("threshold"_a = threshold, "branching_factor"_a = branching_factor,
...
PYBIND11_MODULE(sesame_python, m)
{
    py::class_<Birch>(m, "Birch")
        .def(py::init<double, int, int, int>(), py::arg("threshold") = 0.5,
             py::arg("branching_factor") = 50, py::arg("n_clusters") = 3, py::arg("dim") = 2)
        .def("fit", &Birch::fit)
