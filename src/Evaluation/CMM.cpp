 ...
 */
CMMPoint::CMMPoint(int id, long startTime, long time, std::vector<double> &vec, double a,
...
void CMMDriver::load(const std::vector<PointPtr> &inputs, const std::vector<PointPtr> &predicts,
                     int dim, double time)
{
    // time ? weight ?
    ...
    for (int i = 0; i < inputs.size(); i++)
    {
        ...
        assert(cl != -1);
        CMMPointPtr p =
            std::make_shared<CMMPoint>(predicts.at(i)->getIndex(), (long)predicts.at(i)->getIndex(),
...
#pragma omp parallel for
    ...
    double upperKnn =
        clusters[inputs[i]->clu_id].knnMeanAvg + clusters[inputs[i]->clu_id].knnDevAvg;
    if (avgKnn < upperKnn)
    {
        return 1;
    }
 ...
 * */
void CMM::AnalyseGT(const std::vector<PointPtr> &inputs, const std::vector<PointPtr> &predicts,
                    bool enableClassMerge)
{
    ...
    {
        if (cluster.first != 0) cluster.second.CalcKnn(knnNeighbourhood, inputs);
    }
    // for (int i = 0; i < inputs.size(); ++i)
    //   if (inputs[i]->clu_id != -1) {
...
void CMM::CalcMatch(const std::vector<PointPtr> &inputs, const std::vector<PointPtr> &predicts)
{
...
double CMM::MisplacedError(int i, int fc, int hc, const std::vector<PointPtr> &inputs)
{
    auto real_hc = matchMap[fc];
    if (real_hc == -1) return 1;
    if (real_hc == hc)
        return 0;
    else
    {
...
double CMM::NoiseError(int i, int fc, int hc, const std::vector<PointPtr> &inputs)
{
    auto real_hc = matchMap[fc];
    if (real_hc == -1) return 1;
    if (clusters[real_hc].points.contains(i))
...
double CMM::MissedError(int i, int fc, int hc, const std::vector<PointPtr> &inputs)
{
    // TODO
    return inputs[i]->conn;
...
void CMM::CalcError(const std::vector<PointPtr> &inputs, const std::vector<PointPtr> &predicts)
{
    ...
    for (int i = 0; i < inputs.size(); ++i)
    {
        ...
        if (flag) totalCoverage++;
        if (flag && hc != -1) trueCoverage++;
    }
...
/*here we assume that the data comes every 10 seconds
 * */
double CMM::Evaluate(const std::vector<PointPtr> &inputs, const std::vector<PointPtr> &predicts)
{
#ifndef NDEBUG
    for (int i = 0; i < inputs.size(); i++)
    {
