...
void SESAME::TimeMeter::START_MEASURE() { clock_gettime(CLOCK_REALTIME, &start); }
...
void SESAME::TimeMeter::overallStartMeasure() { clock_gettime(CLOCK_REALTIME, &timer.start); }

void SESAME::TimeMeter::overallEndMeasure() { clock_gettime(CLOCK_REALTIME, &timer.end); }

// the start  of every xx s //TODO the initial is overlap with start Measure
void SESAME::TimeMeter::overallAccMeasure() { clock_gettime(CLOCK_REALTIME, &timer.overallPre); }
...
// end of initial part
void SESAME::TimeMeter::initialEndMeasure() { clock_gettime(CLOCK_REALTIME, &timer.initialTimer); }
...
// end of online part
void SESAME::TimeMeter::onlineEndMeasure() { clock_gettime(CLOCK_REALTIME, &timer.online_timer); }
...
long SESAME::TimeMeter::MeterOutlierDetectionUSEC() { return outlierDetectionTime; }

// start of prune part
void SESAME::TimeMeter::pruneAccMeasure() { clock_gettime(CLOCK_REALTIME, &timer.pruneTimer_pre); }
...
void SESAME::TimeMeter::setOverallTime(long overallT) { overallTime = overallT; }
void SESAME::TimeMeter::setOnlineTime(long onlineT) { onlineTime = onlineT; }
void SESAME::TimeMeter::setDataInsertTime(long dataInsertT) { dataInsertTime = dataInsertT; }
...
void SESAME::TimeMeter::setInitialTime(long initialT) { initialTime = initialT; }
// if possible
void SESAME::TimeMeter::setRefinementTime(long refinementT) { refinementTime = refinementT; }
// if possible
void SESAME::TimeMeter::setSnapshotTime(long snapshotT) { snapshotTime = snapshotT; }
...
void SESAME::TimeMeter::printTime(bool initial, bool snapshot, bool outlierBuffer,
                                  bool finalCluster)
{  //
    ...
    if (finalCluster)  //
        std::cerr << "final cluster: " << finalClusterTime << ", count " << timer.periodicalCluCnt
                  << std::endl;
}
 ...
 */
void SESAME::TimeMeter::breakdown_global(bool initial, bool snapshot, bool outlierBuffer,
                                         bool refine)
{
    MeterInitialUSEC();

    otherTime = overallTime - (dataInsertTime + onlineClusterUpdateTime + outlierDetectionTime);
    SESAME_DEBUG("Overall time is " << overallTime / 1000L
