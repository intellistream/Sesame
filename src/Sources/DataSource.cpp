...
void SESAME::DataSource::load()
{
    ...
    for (int i = 0; i < param.num_points; i++)
    {
        ...
        PointPtr point = DataStructureFactory::createPoint(i, DEFAULT_WEIGHT, param.dim,
        ...
        while (feature != nullptr)
        {
            if (index == param.dim)
            {
                ...
                {  // If cluster id == -1, then it is an noise / outlier
        ...
        for (PointPtr &p : this->input)
        {
            ...
            if (timestamp > duration_cast<nanoseconds>(now - start).count())
            {
                std::this_thread::sleep_for(
                    nanoseconds(timestamp - duration_cast<nanoseconds>(now - start).count()));
            }
