...
void SESAME::DBStream::Init()
{
    ...
    this->weakEntry =
        pow(dbStreamParams.base, (-1) * dbStreamParams.lambda * dbStreamParams.clean_interval);
...
/**
 * @Description: online clustering stage, input data point incrementally and update the MC list and
 * weight adjacency lists,
 * @Param: void
 * @Return: void
 */
    ...
    if (!microClustersList.empty())
    {
        ...
        for (i = 0; i < microClustersList.size(); i++)
        {
            for (j = i + 1; j < microClustersList.size(); j++)
            {
                ...
                if (distance < dbStreamParams.radius) return false;
            }
        }
    }
    return true;
...
void SESAME::DBStream::cleanUp(int nowTime)
{
    ...
    while (iterW != weightedAdjacencyList.end())
    {
        ...
        auto exist1 =
            std::find_if(removeMicroCluster.begin(), removeMicroCluster.end(),
