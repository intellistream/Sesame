    ...
    for (int i = 0; i < this->leafNodes.size(); i++)
    {
        ...
        for (int j = 0; j < BirchParam.dim; j++)
        {
            centroid->setFeatureItem(
                this->leafNodes[i]->getCF()->getLS().at(j) / this->leafNodes[i]->getCF()->getN(),
                j);
        }
...
void SESAME::Birch::pointToClusterDist(SESAME::PointPtr &insertPoint, SESAME::NodePtr &node,
                                       double &dist)
{
    dist                      = 0;
...
void SESAME::Birch::selectChild(vector<SESAME::NodePtr> &children, SESAME::PointPtr &insertPoint,
                                SESAME::NodePtr &node)
{
    double dist = 0;
    double temp = 0;
