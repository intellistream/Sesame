                       ...
                       int32_t k, std::vector<int32_t> *centers, double *cost)
{
    ...
    std::vector<double> min_distance_to_centers(instance.size(),
    ...
    else
    {
        ...
        while (centers->size() < k)
        {
            ...
            for (int pos = 0; pos < instance.size(); ++pos)
            {
                ...
                sum_pow_min_distances += std::pow(min_distance, 2) * instance.at(pos).second;
                min_dist_powers.push_back(std::pow(min_distance, 2) * instance.at(pos).second);
            }
...
vector<double> cost_samples(Random *r, const vector<PointPtr> &samples, int window_size,
...
std::pair<double, double> guess_bounds(const vector<double> &costs)
{
    ...
    double mean     = std::accumulate(std::begin(costs), std::end(costs), double{0}) / costs.size();
    ...
    double mean_sq =
        std::accumulate(std::begin(costs_sq), std::end(costs_sq), double{0}) / costs.size();
...
std::pair<double, double> guess_optimum_range_bounds(Random *r, const vector<PointPtr> &samples,
