...
void SESAME::DStream::Init()
{
    ...
    // double optionB =
    // ((double)this->NGrids-dStreamParams.cm)/((double)this->NGrids-dStreamParams.cl);
    ...
    for (auto iter = 0; iter != this->clusterList.size(); iter++)
    {
        ...
        for (auto &iterGrid : this->clusterList.at(iter).grids)
        {
            for (int iterDim = 0; iterDim < dStreamParams.dim; iterDim++)
            {
                ...
                {
                    point->setFeatureItem(point->getFeatureItem(iterDim) / dStreamParams.dim,
                                          iterDim);
                }
            }
...
void SESAME::DStream::GridListUpdate(std::vector<double> coordinate)
{
    ...
    if (this->gridList.find(grid) == gridList.end())
    {
        ...
        if (this->deletedGrids.find(grid) != deletedGrids.end())
        {
            ...
            characteristicVec = CharacteristicVector(
                pointArrivingTime, this->deletedGrids.find(grid)->second, 1.0, -1, false, dl, dm);
...
void SESAME::DStream::initialClustering()
{
    ...
    while (gridIter != gridList.end())
    {
        ...
        if (characteristicVecOfG.attribute == DENSE)
        {
            ...
            if (std::find(this->clusterList.begin(), this->clusterList.end(), gridCluster) ==
                this->clusterList.end())
                    ...
                    for (const DensityGrid &gridNeighbourhood : grid.getNeighbours())
                    {
                        if (this->gridList.find(gridNeighbourhood) != gridList.end())
                        {
                            ...
                            CharacteristicVector characteristicVec2 =
                                this->gridList.find(gridNeighbourhood)->second;
                            ...
                            if (class1 != class2)
                            {
                                ...
                                {
                                    //// SESAME_INFO("this grid is transitional and is assigned to
                                    ///cluster "<<class1);
...
bool SESAME::DStream::checkIfSporadic(CharacteristicVector characteristicVec)
{
    ...
    if (characteristicVec.getCurrGridDensity(pointArrivingTime, dStreamParams.lambda) <
        outlier_density_thresholdFunction(characteristicVec.densityUpdateTime, dStreamParams.cl,
                                          dStreamParams.lambda, this->NGrids))
    {
        // Check S2 TODO CHANGE REMOVE TIME FROM 0 TO -1
        if (characteristicVec.removeTime == 0 ||
            (pointArrivingTime - ((1 + dStreamParams.beta) * characteristicVec.removeTime)) >= 0)
            return true;
    }
...
double SESAME::DStream::outlier_density_thresholdFunction(int tg, double cl, double lambda,
                                                          int NGrids)
{
    return (cl * (1.0 - pow(lambda, ((double)(pointArrivingTime - tg) + 1.0)))) /
           (NGrids * (1.0 - lambda));
}
