...
void SESAME::DStream::ifReCalculate(PointPtr point)
{
    ...
    if (recalculateN) reCalculateParameter();
}
void SESAME::DStream::reCalculateParameter()
{
    ...
    else
    {
        ...
        double optionB = ((double)this->NGrids - param.cm) / ((double)this->NGrids - param.cl);
...
void SESAME::DStream::GridListUpdate(std::vector<int> coordinate)
{
    ...
    if (it == gridList.end())
    {
        ...
        {
            SESAME_INFO("Once deleted! Try to re-insert into list");
            characteristicVec =
                CharacteristicVector(currentTimeStamp, it2->second, 1.0, -1, false, dl, dm);
...
void SESAME::DStream::initialClustering()
{
    ...
    while (gridIter != gridList.end())
    {
        ...
        putHashMap(newGridList,grid, characteristicVecOfG);
        ++gridIter;
    }
...
bool SESAME::DStream::inspectChangedGrids()
{
    ...
    while (gridIter != gridList.end()&& newGridList.empty())  //&& newGridList.empty()
    {
        ...
        {                                                                 // grid.isVisited=true;
            ...
            putHashMap(gridList,grid,characteristicVec);
            if (characteristicVec.attribute == SPARSE)
                ...
                mergeGridList(newGridList,
                              adjustForTransitionalGrid(grid, characteristicVec, gridClass));
        }
        gridIter++;
        a++;
    }
    // SESAME_INFO("Inspect changes in grids "<<gridList.size());
    // If there are grids in new grid list, update the corresponding grids in grid_list and clean up
    // the cluster list
    if (!newGridList.empty())
    {
        // SESAME_INFO("There are "<<newGridList.size()<<" entries to update from new Grid List to
        // grid_list.");
...
/**
 * Adjusts the clustering of a sparse density grid. Implements lines 5 and 6 from Figure 4 of Chen
 * and Tu 2007.
 *
    ...
    if (gridClass != NO_CLASS)
    {
        ...
        for(auto gridCluster : clusterList)
        {
            if(gridCluster.clusterLabel == gridClass)
            {
               ...
               // newGridList.insert(std::make_pair(grid, characteristicVec));
                putHashMap(newGridList,grid, characteristicVec);
...
SESAME::HashMap SESAME::DStream::reCluster(GridCluster gridCluster)
{
    ...
    while (gcIter != gridCluster.grids.end())
    {
        ...
        putHashMap(newGridList,grid, characteristicVecOfGrid);
        gcIter++;
    }
    ...
    for (GridCluster &cluster : newClusterList) this->clusterList.push_back(cluster);
    return newGridList;
}
                ...
                for (DensityGrid &neighbourGrid : grid.getNeighbours())
                {
                    if (newGridList.find(neighbourGrid) != newGridList.end())
                    {
                        ...
                        if (class1 != class2)
                        {
                            ...
                            // If dgprime is in cluster c', merge c and c' into the larger of the
                            // two
                            if (class2 != NO_CLASS)
                            {
                            ...
                            // If neighbourGrid is transitional and outside of cluster, assign it to
                            // cluster
                            else if (characteristicVec2.isTransitional(dm, dl))
                            {
                                characteristicVec2.label = class1;
                                ...
                                if (it1 != gridListAdjusted.end())
                                  ...
                                  //  gridListAdjusted.insert(std::make_pair(neighbourGrid, characteristicVec2));
                                    putHashMap(gridListAdjusted,neighbourGrid, characteristicVec2);
                                return gridListAdjusted;
                            }
                        }
                    }
...
/**
 * Adjusts the clustering of a dense density grid. Implements lines 8 through 18 from Figure 4 of
 * Chen and Tu 2007.
 *
            ...
            else if (gridClass != NO_CLASS)
            {
                ...
                if (gSize >= ChosenGridSize)
                {
                   ...
                   // newGridList.insert(std::make_pair(gridChosen, cvhChosen));
                    putHashMap(newGridList,gridChosen, cvhChosen);
                ...
                c.grids.find(dghprime) != c.grids.end())
            {
                ...
                if (cvhprime.attribute == TRANSITIONAL)
                {
                   ...
                   // newGridList.insert(std::make_pair(dghprime, cvhprime));
                    putHashMap(newGridList,dghprime, cvhprime);
                }
            }
        }
...
/**
 * Adjusts the clustering of a transitional density grid. Implements lines 20 and 21 from Figure 4
 * of Chen and Tu 2007.
 *
 ...
 * this iteration
 */
SESAME::HashMap SESAME::DStream::adjustForTransitionalGrid(DensityGrid grid,
                                                           CharacteristicVector characteristicVec,
                                                           int gridClass)
{
    // Among all neighbours of dg, find the grid h whose cluster ch has the largest size
    ...
    // System.out.println(" Chosen neighbour is from cluster "+hChosenClass+", dgClass is
    // "+dgClass+".");
...
SESAME::HashMap SESAME::DStream::mergeNewClusters(SESAME::HashMap newGridList, int smallCluster,
                                                  int bigCluster)
{
...
bool SESAME::DStream::checkIfSporadic(CharacteristicVector characteristicVec)
{
    ...
    if (characteristicVec.getCurrGridDensity(currentTimeStamp, param.lambda) <
        outlier_density_thresholdFunction(characteristicVec.densityUpdateTime, param.cl,
                                          param.lambda, this->NGrids))
    {
 ...
 */
double SESAME::DStream::outlier_density_thresholdFunction(int tg, double cl, double lambda,
                                                          int NGrids)
{
...
SESAME::HashMap SESAME::DStream::cleanNewClusters(SESAME::HashMap newGridList)
{
    ...
    {
        if (cluster.grids.empty()) toRemove.push_back(cluster);
    }
...
void SESAME::DStream::putHashMap(HashMap gList, const DensityGrid& g, CharacteristicVector cv)
{
