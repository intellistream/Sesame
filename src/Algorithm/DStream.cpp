...
void SESAME::DStream::runOnlineClustering(PointPtr input) {
  ...
  else
  {
    ...
    if (recalculateN)
    {
      ...
      int timeInterval = (int)(clock()-startTime)/CLOCKS_PER_SEC;
      if ((int) (clock()-startTime)/CLOCKS_PER_SEC % gap == 0)
      {
        if (timeInterval == gap)//TODO if never fetch?
         initialClustering();
        else
        {
          removeSporadic();
