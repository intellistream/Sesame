    ...
    for (iterW = weightedAdjacencyList.begin(); iterW != weightedAdjacencyList.end(); iterW++)
    {
            ...
            iterW->first.microCluster2->weight >= min_weight)
        {
            double val = 2 * iterW->second->weight /
                         (iterW->first.microCluster1->weight + iterW->first.microCluster2->weight);
            if (val > min_weight)
            {
    ...
    const std::vector<SESAME::MicroClusterPtr> &microClusters)
{
    ...
    for (iter = connecvtivityGraphId.begin(); iter != connecvtivityGraphId.end(); iter++)
    {
        ...
        if (microClusterKey != microClusters.end() && !(*microClusterKey)->visited)
        {
            ...
            {
                auto microClusterElement = std::find_if(
                    microClusters.begin(), microClusters.end(),
                    [&](const MicroClusterPtr &mc) { return mc->id.front() == iterValue; });
...
std::vector<SESAME::PointPtr> SESAME::ConnectedRegions::ResultsToDataSink()
{
    ...
    {  // initialize pseudo point of macro clusters
        PointPtr point =
            DataStructureFactory::createPoint(iter, 0, finalClusters.at(iter).front()->dim, 0);
        ...
        for (auto j = 0; j < finalClusters.at(iter).size(); j++)
        {
            ...
            {
                if (j == 0) point->setFeatureItem(0, a);
                point->setFeatureItem(
                    point->getFeatureItem(a) + finalClusters.at(iter).at(j)->centroid.at(a), a);
