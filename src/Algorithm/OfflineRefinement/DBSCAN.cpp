                                  ...
                                  int clusterID) const
{
    ...
    else
    {
        ...
        for (int iterSeeds = 0; iterSeeds < clusterSeeds.size(); iterSeeds++)
        {
            ...
            if (judgeCorePoint(input.at(index),
                               point))  // check if the seed point in input is the core point?
                indexCorePoint = iterSeeds;
        }
        clusterSeeds.erase(clusterSeeds.begin() + indexCorePoint);

        for (std::vector<int>::size_type i = 0, currentSize = clusterSeeds.size(); i < currentSize;
             i++)  // ++i or i++?
        {
            std::vector<int> clusterNeighbors = calculateCluster(input, input.at(clusterSeeds[i]));
                                                  ...
                                                  PointPtr &point) const
{
    ...
    for (int i = 0; i < input.size(); i++)
    {
        if (calculateEluDistance(point, input[i]) <= epsilon) clusterIndex.push_back(i);
    }
    return clusterIndex;
}
...
bool SESAME::DBSCAN::judgeCorePoint(PointPtr &point, PointPtr &other)
{
    ...
    for (int i = 0; i < point->getDimension(); i++)
    {
        if (point->getFeatureItem(i) != other->getFeatureItem(i)) corePoint = false;
    }
    return corePoint;
}
...
void SESAME::DBSCAN::produceResult(std::vector<PointPtr> &input, SESAME::DataSinkPtr sinkPtr)
{
    for (auto el : input)
    {
        if (el->getClusteringCenter() == UNCLASSIFIED or NOISE)
        {  // not noise or unclassified
