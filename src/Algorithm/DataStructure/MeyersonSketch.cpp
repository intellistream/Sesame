...
MeyersonSketch::MeyersonSketch(const param_t &param) : param(param), r(param.seed)
{
    max_sketch_size_ =
        pow(2, 2 * 2 + 1) * 4. * param.k * (1. + log(param.sliding * 3)) * (1.0 + 1. / 0.5);
}
...
MeyersonSketch::NodePtr MeyersonSketch::Insert(PointPtr input)
{
    if (!has_sampled)
    {
        ...
        if (samples.size() >= param.num_samples * param.sliding)
        {
            ...
            distance_denominator_ = (upper_bound) / (param.k * (1. + log(param.sliding * 3)));
            NodePtr res;
            for (auto p : samples)
            {
                res = Process(p);
