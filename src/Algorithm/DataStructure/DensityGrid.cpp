...
double SESAME::DensityGrid::getInclusionProbability(Point point)
{
    for (int i = 0; i < this->dims; i++)
    {
        if ((int)point.getFeatureItem(i) != this->coordinates[i]) return 0.0;
    }
    return 1.0;
}
...
bool SESAME::DensityGrid::operator==(DensityGrid &gridOther) const
{
    ...
    if (this->dims != gridOther.dims) return false;
    for (int i = 0; i < this->dims; i++)
    {
        if (this->coordinates[i] != gridOther.coordinates[i]) return false;
    }
    return true;
}