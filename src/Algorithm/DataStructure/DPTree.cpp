...
void SESAME::DPTree::Init(std::vector<SESAME::DPNodePtr> &clus, int size, double minRho,
...
SESAME::DPNodePtr SESAME::DPTree::findNN(SESAME::PointPtr p, double coef, int opt, double time)
{
    int index   = 0;
    double dis  = 0;
    auto minDis = DBL_MAX;
...
double SESAME::DPTree::computeAlpha(double minDelta)
{
    ...
    double alpha = (up * (down - delta1) * (m * up + delta1)) /
                   (avg * avg * (delta1 - up) * (n - 1) + (down - delta1) * up * (m * up + delta1));
    double alpha2 =
        ((delta2 - down) * up * (m * up - delta2)) /
        ((delta2 - down) * up * (m * up - delta2) + avg * avg * (up - delta2) * (n + 1));
    if (alpha < alpha2)
    {
...
void SESAME::DPTree::adjustCluster(std::unordered_set<SESAME::ClusterPtr> &clusters)
{
    ...
    else if (Clus[0]->GetCluster() == nullptr)
    {  // there is new cluster center appearing
...
void SESAME::DPTree::SetClus(std::vector<SESAME::DPNodePtr> &clus) { Clus = clus; }
