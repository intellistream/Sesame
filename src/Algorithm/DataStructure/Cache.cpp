...
#include <cfloat>
#include <cmath>
bool cmp(SESAME::DPNodePtr &c1, SESAME::DPNodePtr &c2) { return c1->GetRho() > c2->GetRho(); }
...
void SESAME::Cache::compDeltaRho(double time)
{
    ...
    {
        buffer[i]->SetRho(
            (float)(pow(a, lamd * (time - buffer[i]->GetLastTime())) * buffer[i]->GetRho()));
        clus[i] = buffer[i];
    }
...
void SESAME::Cache::getDPTree(double minRho, double minDelta, SESAME::DPTreePtr &dpTree,
...
void SESAME::Cache::SetBuffer(std::vector<DPNodePtr> &buffer) { Cache::buffer = buffer; }
std::vector<SESAME::DPNodePtr> &SESAME::Cache::GetClus() { return clus; }
void SESAME::Cache::SetClus(std::vector<DPNodePtr> &clus) { Cache::clus = clus; }
