...
int SESAME::CF::getN() const {
  return this->NumberOfNodes;
}

void SESAME::CF::setN(int n) {
  this->NumberOfNodes = n;
}
std::vector<double> SESAME::CF::getLS() const {
  return this->LS;
}

std::vector<double> SESAME::CF::getSS() const {
  return this->SS;
}
...
void SESAME::CF::setLS(std::vector<double> & newLs) {
  if(this->getLS().empty()) {
    ...
    if(this->getLS().size() != newLs.size()) {
      std::cout << "Size Error: CF's LS size: " << this->getLS().size() << ", newLS's size: " << newLs.size();
    } else {
...
void SESAME::CF::setSS(std::vector<double> & newSs) {
  if(this->getSS().empty()) {
    ...
    if(this->SS.size() != newSs.size()) {
      std::cout << "Size Error: CF's SS size: " << this->getSS().size() << ", newSs's size: " << newSs.size();
    } else {
