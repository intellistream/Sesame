...
void CFNode::setChildren(std::vector<NodePtr> children) { this->children = children; }
CFNode::CFNode()
{
    ...
    else
    {
        while (1)
        {
            if (curNode->IsLeaf())
            {
                ...
                if (point->L2Dist(centroid) <= distance_threshold)
...
ClusteringFeaturesTree::NodePtr ClusteringFeaturesTree::Insert(NodePtr node)
{
    ...
    while (1)
    {
        ...
        if (curNode->IsLeaf())
        {
            ...
            if (center->L2Dist(centroid) <= distance_threshold)
            {  // concept drift detection
                // whether the new radius is lower than threshold T
                // timerMeter.dataInsertAccMeasure();
...
void ClusteringFeaturesTree::Remove(NodePtr node)
{
    ...
    if (parent != nullptr) parent->RemoveChild(node);
    const auto [first, last] =
        std::ranges::remove_if(clusters_, [node](auto &cluster) { return cluster == node; });
...
template <typename T>
ClusteringFeaturesTree::NodePtr ClusteringFeaturesTree::backwardEvolution(NodePtr node, T input)
{
    ...
    else
    {
        ...
        else
        {
            ...
            while (true)
            {
                ...
                auto adjMatrix = CalcAdjMatrix(broNodes);  //  calculate the distance between
                ...
                for (int i = 0; i < broNodes.size(); i++)
                {
                    if (i != seedA && i != seedB)
                    {
                        if (adjMatrix[i][seedA] < adjMatrix[i][seedB])
                        {
                            ...
                            newParentA->Update(broNodes[i]);  // since the brother nodes list
...
std::vector<ClusteringFeaturesTree::NodePtr> &ClusteringFeaturesTree::clusters()
{
    return clusters_;
}
...
std::vector<ClusteringFeaturesList::NodePtr> &ClusteringFeaturesList::clusters()
{
    return clusters_;
}
