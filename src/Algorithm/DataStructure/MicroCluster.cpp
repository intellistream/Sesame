...
bool SESAME::MicroCluster::judgeMerge(MicroClusterPtr other)
{
    ...
    {
        if (std::find(this->id.begin(), this->id.end(), other->id[i]) == id.end()) merge = false;
    }
    return merge;
}
...
{
    if (weight < (2 * num_last_arr)) return getMutime();
...
SESAME::PointPtr SESAME::MicroCluster::getCenter()
{
    ...
    for (int i = 0; i < centroid.size(); i++) center->setFeatureItem(LS[i] / weight, i);
    return center;
}
...
SESAME::dataPoint SESAME::MicroCluster::getVarianceVector()
{
    ...
    for (int i = 0; i < dim; i++)
    {
        ...
        if (datapoint[i] <= 0.0) datapoint[i] = MIN_VARIANCE;
    }
    return datapoint;
}
...
void SESAME::MicroCluster::decayWeight(double decayFactor) { this->weight *= decayFactor; }
