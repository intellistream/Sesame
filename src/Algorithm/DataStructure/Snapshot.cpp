...
SESAME::Snapshot::~Snapshot() { std::vector<MicroClusterPtr>().swap(this->microClusters); }
SESAME::SnapshotPtr SESAME::Snapshot::findSnapshot(QueueOrderSnapshot orderSnapShots,
            ...
            for (unsigned int j = 0; j < num_clusters; j++)
            {
                ...
                else
                {
                    ...
                    if (std::find(snapshotCurrent->microClusters[i]->id.begin(),
                                  ...
                                  clusterIdLandmark) != snapshotCurrent->microClusters[i]->id.end())
                        snapshotCurrent->microClusters[i]->subtractClusterVector(
                            snapshotLandmark->microClusters[j]);
                }
            }
...
SESAME::SnapshotPtr SESAME::Snapshot::copy() { return std::make_shared<Snapshot>(*this); }
