...
void SESAME::StreamKM::Init()
{
    ...
    this->window->windowManager.numberOfWindow =
        ceil(log((double)this->StreamKMParam.num_points / (double)this->StreamKMParam.windowSize) /
             log(2)) +
        2;
    ...
    SESAME_DEBUG("Created manager with " << this->window->windowManager.numberOfWindow
...
void SESAME::StreamKM::RunOffline(DataSinkPtr sinkPtr)
{
    ...
    for (int i = 0; i < parNumber; i++)
    {
        ...
        {
            this->streamingCoreset[i]->setFeatureItem(this->streamingCoreset[i]->getFeatureItem(j) /
    ...
    if (param.run_offline)
    {
        ...
        this->km.runKMeans(this->streamingCoreset.size(), parNumber, centers,
                           this->streamingCoreset, oldGroups, newGroups, this->StreamKMParam.seed,
                           true);
...
void SESAME::LandmarkWindow::CoresetTree::unionTreeCoreset(int k, int n_1, int n_2,
                ...
                for (l = 0; l < setA[i]->getDimension(); l++)
                {
                    if (setA[i]->getWeight() != 0.0)
                    {
                        centres[index]->setFeatureItem(
                            setA[i]->getFeatureItem(l) + centres[index]->getFeatureItem(l), l);
                    }
                }
            }
        }
        else
        {
            ...
            if (centres[index]->getIndex() != setB[i - n_1]->getIndex())
            {
                ...
                for (l = 0; l < setB[i - n_1]->getDimension(); l++)
                {
                    ...
                    {
                        centres[index]->setFeatureItem(
                            setB[i - n_1]->getFeatureItem(l) + centres[index]->getFeatureItem(l),
                            l);
                    }
                }
            }
...
void SESAME::LandmarkWindow::CoresetTree::constructRoot(TreeNodePtr root,
                                                        std::vector<PointPtr> &setA,
                                                        std::vector<PointPtr> &setB, int n_1,
                                                        int n_2, PointPtr centre, int centreIndex)
{
...
void SESAME::LandmarkWindow::CoresetTree::treeNodeTargetFunctionValue(TreeNodePtr node)
{
    // loop counter variable
    int i;
...
SESAME::TreeNodePtr SESAME::LandmarkWindow::CoresetTree::selectNode(TreeNodePtr root)
{
 ...
 * TODO: Why hard-code??
 */
SESAME::PointPtr SESAME::LandmarkWindow::CoresetTree::chooseCentre(TreeNodePtr node)
{
...
double SESAME::LandmarkWindow::CoresetTree::treeNodeCostOfPoint(TreeNodePtr node, PointPtr p)
{
    if (p->getWeight() == 0.0)
    {
        return 0.0;
 ...
 * @return
 */
double SESAME::LandmarkWindow::CoresetTree::treeNodeSplitCost(TreeNodePtr node, PointPtr centreA,
                                                              PointPtr centreB)
{
    // loop counter variable
    int i;
 ...
 * @return
 */
SESAME::PointPtr SESAME::LandmarkWindow::CoresetTree::determineClosestCentre(PointPtr point,
                                                                             PointPtr centreA,
                                                                             PointPtr centreB)
{
    // loop counter variable
