...
bool SESAME::V2::checkoutOutlier(SESAME::PointPtr &point)
{
    ...
    if (minDIstance > this->V2Param.outlier_distance_threshold)
        return false;
    else
        return true;
}
...
void SESAME::V2::RunOffline(DataSinkPtr sinkPtr)
{
    ...
    this->dbscan->Run(V2Param, online_centers, sinkPtr);

    //  this->dbscan->run(online_centers);
    //
    //  this->dbscan->produceResult(online_centers, sinkPtr);
    // store outlier nodes
...
void SESAME::V2::pointToClusterDist(SESAME::PointPtr &insertPoint, SESAME::NodePtr &node,
                                    double &dist)
{
    dist                      = 0;
...
void SESAME::V2::selectChild(vector<SESAME::NodePtr> &children, SESAME::PointPtr &insertPoint,
                             SESAME::NodePtr &node)
{
    double dist = 0;
    double temp = 0;
...
void SESAME::V2::calculateCorDistance(vector<vector<double>> &distance,
