...
void SESAME::V1::RunOffline(DataSinkPtr sinkPtr)
{
    ...
    for (int i = 0; i < this->clusterNodes.size(); i++)
    {
        ...
        online_centers.push_back(centroid->copy());
    }
    // here if num_clusters is lower than online cluster center number, we choose
    // to skip offline.
    this->km->Run(this->param, online_centers, sinkPtr);
    // store outlier nodes
...
SESAME::V1::V1(param_t &cmd_params) { this->param = cmd_params; }
SESAME::V1::~V1() {}
...
void SESAME::V1::pointToClusterDist(SESAME::PointPtr &insertPoint, SESAME::NodePtr &node,
                                    double &dist)
{
    dist                      = 0;
...
void SESAME::V1::selectChild(vector<SESAME::NodePtr> &children, SESAME::PointPtr &insertPoint,
                             SESAME::NodePtr &node)
{
    double dist = 0;
    double temp = 0;
