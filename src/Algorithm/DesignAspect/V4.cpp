...
bool SESAME::V4::checkoutOutlier(SESAME::PointPtr &point)
{
    ...
    if (minDistance > this->V4Param.outlier_distance_threshold)
        return false;
    else
        return true;
}
...
void SESAME::V4::RunOnline(const SESAME::PointPtr input)
{
    ...
    if (this->slidingWindowPoints.size() == this->V4Param.slidingCount)
    {
        ...
        this->slidingWindowPoints.erase(
            this->slidingWindowPoints.begin());  // delete the first come point
        this->SlidingWindowNodes.erase(
            this->SlidingWindowNodes.begin());  // delete the node inserted by first come point
    }
    else
    {
        this->slidingWindowPoints.push_back(input->copy());
...
void SESAME::V4::pointToClusterDist(SESAME::PointPtr &insertPoint, SESAME::NodePtr &node,
                                    double &dist)
{
    dist                      = 0;
...
void SESAME::V4::selectChild(vector<SESAME::NodePtr> &children, SESAME::PointPtr &insertPoint,
                             SESAME::NodePtr &node)
{
    double dist = 0;
    double temp = 0;
...
void SESAME::V4::calculateCorDistance(vector<vector<double>> &distance,
