...
void SESAME::V10::InitDP(double time) {
  ...
  this->minRho = this->V10Param.beta / (1 - pow(this->V10Param.alpha, this->V10Param.lambda));
  SESAME_DEBUG("minRho = " << this->minRho);

  this->deltaT = (log(1 - pow(this->V10Param.alpha, this->V10Param.lambda)) / log(this->V10Param.alpha) -
      log(this->V10Param.beta) / log(this->V10Param.alpha))/ this->V10Param.lambda;
  //		double deltaT = 100;
  SESAME_DEBUG("deltaT = " << this->deltaT);
  outres->setTimeGap(this->deltaT);
  cache->getDPTree(this->minRho, this->V10Param.delta, dpTree, outres, clusters);
...
SESAME::DPNodePtr SESAME::V10::streamProcess(SESAME::PointPtr p, int opt, double time) {
  ds_timer.Tick();
  double coef = pow(this->V10Param.alpha, this->V10Param.lambda * (time - dpTree->GetLastTime()));
  dpTree->SetLastTime(time);
...
double SESAME::V10::computeAlpha() {
  return dpTree->computeAlpha(this->V10Param.delta);
}
...
void SESAME::V10::RunOnline(SESAME::PointPtr input) {
  if(input->getIndex() >= this->V10Param.landmark){
    ...
    if(input->getIndex() % 100 == 0 && this->isInit) {
      ds_timer.Tick();
