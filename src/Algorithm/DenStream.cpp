    ...
    for (int i = 0; i < denStreamParams.buf_sizeSize; i++)
    {
        if (initData.at(i)->getClusteringCenter() == noVisited)
        {
            ...
            MicroClusterPtr newMicroCluster = SESAME::DataStructureFactory::createMicroCluster(
                denStreamParams.dim, pMicroClusterIndex);
...
void SESAME::DenStream::pointsNearCorePoint(vector<PointPtr> &initData, std::vector<int> pointIndex,
...
void SESAME::DenStream::Init()
{
    ...
    if (this->Tp > 1000 || this->Tp <= 0) this->Tp = 1;
    sum_timer.Tick();
}
            ...
            if (!oMicroClusters.empty())
            {
                for (int iter = 0; iter < oMicroClusters.size(); iter++)
                {
                    double a = -(denStreamParams.lambda) *
                               (pointArrivingTime - oMicroClusters.at(iter)->createTime + this->Tp);
                    double b = -denStreamParams.lambda * this->Tp;
                    double Xi =
                        (pow(denStreamParams.base, a) - 1) / (pow(denStreamParams.base, b) - 1);
...
void SESAME::DenStream::merge(PointPtr dataPoint)
{
    ...
    {
        oMicroClusterIndex++;
        MicroClusterPtr newOMicroCluster =
            DataStructureFactory::createMicroCluster(denStreamParams.dim, oMicroClusterIndex);
...
bool SESAME::DenStream::mergeToMicroCluster(PointPtr dataPoint,
...
bool SESAME::DenStream::mergeToOMicroCluster(PointPtr dataPoint,
                                             std::vector<MicroClusterPtr> microClusters)
{
    out_timer.Tick();
    MicroClusterPtr MC = nearestNeighbor(dataPoint, microClusters);
...
int SESAME::DenStream::findIndex(std::vector<MicroClusterPtr> &microClusters, MicroClusterPtr MC)
{
    ...
    if (ret != microClusters.end()) return ret - microClusters.begin();
    return -1;
}

SESAME::MicroClusterPtr SESAME::DenStream::nearestNeighbor(
                                            ...
                                            vector<PointPtr> &points)
{
    ...
    {
        PointPtr point = DataStructureFactory::createPoint(i, microClusters.at(i)->weight,
                                                           microClusters.at(i)->centroid.size(), 0);
