...
void SESAME::CluStream::initOffline(vector<PointPtr> &initData, vector<PointPtr> &initialData)
{
    ...
    {
        microClusters.push_back(DataStructureFactory::createMicroCluster(CluStreamParam.dim, i));
    }
    std::vector<PointPtr> centers;
    std::vector<std::vector<PointPtr>> oldGroups, newGroups;
    this->kmeans->runKMeans(CluStreamParam.num_clusters, CluStreamParam.buf_size, centers, initData,
                            oldGroups, newGroups, CluStreamParam.seed, true);
...
double SESAME::CluStream::calRadius(MicroClusterPtr closestCluster)
{
    ...
    if (closestCluster->weight == 1)
    {
        ...
        for (int i = 0; i < this->CluStreamParam.num_clusters; i++)
        {
            ...
            double dist =
                distance(microClusters[i]->getCentroid(), centroid, this->CluStreamParam.dim);
...
void SESAME::CluStream::MergeCreateCluster(PointPtr data)
{
    ...
    {  // O(n(n+1)/2)
        ...
        {
            double dist =
                distance(centroidA, microClusters[j]->getCentroid(), this->CluStreamParam.dim);
            if (dist < minDistance)
            {
                minDistance = dist;
                closestA    = i;
                                            ...
                                            vector<PointPtr> &points) const
{
    for (int i = 0; i < this->CluStreamParam.num_clusters; i++)
    {
        PointPtr point = DataStructureFactory::createPoint(i, microClusters[i]->weight,
                                                           microClusters[i]->centroid.size(), 0);
...
void SESAME::CluStream::RunOffline(SESAME::DataSinkPtr sinkPtr)
{
    ...
    if (this->CluStreamParam.offline_time_window > elapsedTime) landmarkTime = 0;
    ...
    subtractMiroCluster = DataStructureFactory::createSnapshot(microClusters, elapsedTime);

    // The offline is to observe a process of data stream clustering
    if (CluStreamParam.offline_time_window != 0)
    {  //
        landmarkSnapshot =
            SESAME::Snapshot::findSnapshot(window->orderSnapShots, landmarkTime, elapsedTime,
        ...
        if (landmarkSnapshot->elapsedTime == -1) landmarkSnapshot = subtractMiroCluster;

        subtractMiroCluster = SESAME::Snapshot::substractSnapshot(
            subtractMiroCluster, landmarkSnapshot, this->CluStreamParam.num_clusters);
    }
