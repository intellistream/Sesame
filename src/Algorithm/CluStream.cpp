...
void SESAME::CluStream::initOffline(vector<PointPtr> &initData, vector<PointPtr> &initialData)
{
 ...
 for(int i=0;i<CluStreamParam.clusterNumber;i++)
  {
    microClusters.emplace_back(CluStreamParam.dimension,i);
  }
...
{ // 1. Determine closest clusters
  MicroCluster * closestCluster;//TODO think how to remove it
...
void SESAME::CluStream::insertIntoCluster(PointPtr data, MicroCluster* operateCluster)
{
 // SESAME_INFO("This data fits");
  pointsFitted++;
  int timestamp=(int)((clock()-startTime)/CLOCKS_PER_SEC);
  operateCluster->insert(std::move(data), timestamp);
}
...
void SESAME::CluStream::deleteCreateCluster(PointPtr data)
{
  ...
  for (int i = 0; i <this->CluStreamParam.clusterNumber; i++) {
    if (microClusters[i].getRelevanceStamp(this->CluStreamParam.lastArrivingNum) < threshold)
    {
      ...
      microClusters[i].reInit(newId,data,elapsedTime);
      pointsForgot++;
...
void SESAME::CluStream::MergeCreateCluster(PointPtr data)
{
 // SESAME_INFO("Micro cluster needs to merge");
  ...
  int elapsedTime=(int)((clock()-startTime)/CLOCKS_PER_SEC);
  microClusters[closestB].reInit(newId,std::move(data),elapsedTime);
  pointsMerged++;
...
void SESAME::CluStream::runOfflineClustering(const std::vector<PointPtr> &input, vector<PointPtr> &output)
{
    ...
    SESAME::Snapshot landmarkSnapshot;
    SESAME::Snapshot substractMiroCluster;
  //If offlineTimeWindow ==0, Only Observe the end results of micro clusters
    ...
    substractMiroCluster.elapsedTime=(int)((now-startTime)/CLOCKS_PER_SEC);
  ...
  for(int i=0;i<CluStreamParam.clusterNumber;i++)
  {
    ...
    std::copy(substractMiroCluster.microClusters[i].centroid.begin(), substractMiroCluster.microClusters[i].centroid.end(), std::ostream_iterator<double>(result, " "));
   ...
   // SESAME_INFO("The micro cluster centroid size "<<substractMiroCluster.microClusters[i].centroid.size()<<"weight is "<<substractMiroCluster.microClusters[i].clusterNum);
   // SESAME_INFO("Centroid is :"<<result.str());
   // SESAME_INFO("ID is :"<<re2.str());
  }
