...
void SESAME::LandmarkWindow::insertPoint(PointPtr point)
{
    ...
    this->windowManager.windows[0].points[cursize] = point->copy();  //   .copy(point);
    ...
    std::vector<PointPtr> &coreset)
{
    ...
    else
    {
        ...
        for (i = 0; i < this->windowManager.numberOfWindow; i++)
        {
            if (this->windowManager.windows[i].cursize == this->windowManager.maxWindowSize)
            {
        ...
        for (j = i + 1; j < this->windowManager.numberOfWindow; j++)
        {
            if (this->windowManager.windows[j].cursize != 0)
            {
                ...
                this->tree->unionTreeCoreset(
                    this->windowManager.maxWindowSize, this->windowManager.maxWindowSize,
                    this->windowManager.maxWindowSize, this->windowManager.windows[j].points,
...
void SESAME::LandmarkWindow::pyramidalWindowProcess(int elapsedTime,
                                                    const SESAME::MicroClusters &microClusters)
{
    int i = -1;
    if (elapsedTime > 0)
    {
        ...
        while (++i >= 0)  //++i>=0
        {
            if (this->pyramidalWindow.currentOrder >= i &&
                elapsedTime % (int)(pow(this->pyramidalWindow.time_interval, i)) == 0)
            {
                if (elapsedTime % (int)(pow(this->pyramidalWindow.time_interval, i + 1)) != 0)
                {
