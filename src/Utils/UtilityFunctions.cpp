...
UtilityFunctions::UtilityFunctions() { mti = N + 1; /* mti==N+1 means mt[N] is not initialized */ }
...
void UtilityFunctions::groupByCenters(const vector<PointPtr> &input,
                                      const vector<PointPtr> &centers, vector<PointPtr> &output,
                                      int dim)
{
    auto n = input.size();
    for (int i = 0; i < n; i++) output.push_back(input[i]->copy());
...
bool checkIfFileExists(const string &filePath) { return access(filePath.c_str(), 0) == 0; }
