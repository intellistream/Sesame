...
public:
    ...
    explicit ApproxTimeCountKeeper(double epsilon) : epsilon_(epsilon) { assert(epsilon_ > 0); }
...
public:
    ...
    SummaryAlg(Random *r, int64_t window, int32_t k, double lambda)
        : r(r), window_(window), k_(k), lambda_(lambda), is_empty_(true), first_element_time_(-1)
    {}
    virtual ~SummaryAlg() {}
    ...
    virtual void solution_left_composition(const SummaryAlg &summary_A, int64_t window_begin,
                                           vector<PointPtr> *sol, double *value) const = 0;

    // Output the result of composing this summary (as summary A) with  and empty
    // summary_B on the right.
    virtual void solution_right_composition_with_empty(int64_t window_begin, vector<PointPtr> *sol,
...
public:
    ...
    {
        max_sketch_size_ =
            std::pow(2, 2 * 2 + 1) * 4. * k * (1. + std::log(window * 3)) * (1.0 + 1. / 0.5);
        distance_denominator_ = (optimum_upperbound_guess) / (k * (1. + std::log(window * 3)));
    ...
    void solution(vector<PointPtr> *centers_solution, double *cost_solution) const override
    {
        ...
        sketches_.at(best_pos).solution(/* after_time=*/0, centers_solution, cost_solution);
    }

    void solution_left_composition(const SummaryAlg &summary_A, int64_t window_begin,
                                   vector<PointPtr> *sol, double *value) const override
    {
        sol->clear();
        const KMeansSummary &summary_A_type = dynamic_cast<const KMeansSummary &>(summary_A);
        ...
        for (int i = 0; i < kSketchNumber; i++)
        {
            double cost;
            vector<PointPtr> centers;
            bool ok = sketches_.at(i).compose_left_solution(summary_A_type.sketches_.at(i),
                                                            window_begin, &centers, &cost);
            if (ok && cost < min_cost)
            {
                min_cost = cost;
                best_pos = i;
        ...
        sketches_.at(best_pos).compose_left_solution(summary_A_type.sketches_.at(best_pos),
                                                     window_begin, sol, value);
    }

    void solution_right_composition_with_empty(int64_t window_begin,
    ...
    public:
        ...
        // "after_time".
        bool solution(const int after_time, std::vector<PointPtr> *centers, double *cost) const
        {
            centers->clear();
            *cost = 0.0;
            ...
            {
                instance.push_back(
                    std::make_pair(centers_[i], multiplicities_[i].total_after_time(after_time)));
            }
            ...
            for (int i = 0; i < centers_.size(); i++)
            {
                ...
                cost_instance +=
                    2.0 * min_distance * costs_sum_dist_.at(i).total_after_time(after_time);
            }
            *cost = cost_instance;
        ...
        bool compose_left_solution(const MeyersonSketch &summary_A, int64_t window_begin,
                                   std::vector<PointPtr> *centers, double *cost) const
        {
            centers->clear();
            ...
            assert(summary_A.multiplicities_.size() == summary_A.costs_sum_dist_.size());
            assert(summary_A.multiplicities_.size() == summary_A.costs_sum_sq_dist_.size());
            instance.reserve(summary_A.multiplicities_.size() + multiplicities_.size());
            for (int i = 0; i < summary_A.multiplicities_.size(); i++)
            {
                instance.push_back(
                    std::make_pair(summary_A.centers_.at(i),
            ...
            {
                instance.push_back(
                    std::make_pair(centers_.at(i), multiplicities_.at(i).total_after_time(0)));
            }
            double cost_instance = 0;
            ...
            {
                cost_instance += summary_A.costs_sum_sq_dist_.at(i).total_after_time(window_begin);
            }
            for (int i = 0; i < costs_sum_sq_dist_.size(); i++)
            {
            ...
            for (int i = 0; i < centers_.size(); i++)
            {
                ...
                cost_instance += 2.0 * min_distance * costs_sum_dist_.at(i).total_after_time(0);
            }
            for (int i = 0; i < summary_A.centers_.size(); i++)
            {
                ...
                for (const auto &center : *centers)
                {
                    min_distance = std::min(min_distance, summary_A.centers_[i]->L2Dist(center));
                }
...
protected:
    ...
    void reset_impl() override
    {
        ...
        for (int i = 0; i < kSketchNumber; i++)
        {
            MeyersonSketch sketch(r, max_sketch_size_, distance_denominator_, kEpsilonMult, k_);
...
public:
    ...
    void solution(vector<PointPtr> *solution, double *value)
    {
        ...
        for (const auto &threshold_alg : threshold_algs_)
        {
                    ...
                    window_handler_.begin_window())
            {
                ...
                else
                {
                    threshold_alg.get_summary_B()->solution_left_composition(
                        *threshold_alg.get_summary_A(), window_handler_.begin_window(), solution,
                        value);
                    if (*value < min_cost)
                    {
                        min_cost      = *value;
            ...
            {  // backup for the case with no guarantees.
                if (!threshold_alg.get_summary_A()->is_empty() &&
                    threshold_alg.get_summary_A()->first_element_time() < oldest_begin_first)
                {
                    oldest_begin_first = threshold_alg.get_summary_A()->first_element_time();
                    threshold_alg.get_summary_B()->solution_left_composition(
