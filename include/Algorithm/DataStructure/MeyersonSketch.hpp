...
public:
    struct Node : std::enable_shared_from_this<Node>
    {
        ...
        Node(SktchPtr s, PointPtr p) : dim(p->getDimension()), cf(p->getDimension()) { Update(p); }
        Node(PointPtr p) : dim(p->getDimension()), cf(p->getDimension()) { Update(p); }
        PointPtr Centroid()
        {
...
std::pair<double, double> guess_optimum_range_bounds(Random *r,
                                                     const std::vector<PointPtr> &samples,
