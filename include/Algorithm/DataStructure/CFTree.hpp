...
private:
    ...
    const double distance_threshold;  // threshold radius of each sub cluster in leaf nodes
    const int dim;
    int leafMask = 0;
...
private:
    template <typename T>
    NodePtr backwardEvolution(NodePtr node, T point);
    NodePtr root_;
...
public:
    struct Node : std::enable_shared_from_this<Node>
    {
        ...
        void ClearParents()
        {
            if (parent != nullptr) parent->index = -1;
        }
        void Update(PointPtr point)
        {
            ...
            if (cf.num == 0)
            {
                if (tree != nullptr) tree->Remove(shared_from_this());
            }
        }
        ...
        {
            std::string str =
                Prefix(d) + std::to_string(index) + ":" + std::to_string(cf.num) + "\n";
            return str;
        }
    };
};
...
public:
    ...
    NodePtr Insert(PointPtr point);
    NodePtr Insert(NodePtr node);
    void ForEach(std::function<void(NodePtr)> func) { std::ranges::for_each(clusters_, func); }
    void Init() {}
    std::vector<NodePtr> &clusters();
    void Remove(NodePtr node);
