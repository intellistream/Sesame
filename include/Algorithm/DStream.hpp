...
1.Since it is unrealistic to set the number of grids in the total feature space with a fixed value ahead of time,
  in this implementation, we timely adjust N during the clustering procedure: pi_(maxVals - minVals) / grid_width.
2.For simplicity, we directly use the data index as the its arriving timestamp.
**/
