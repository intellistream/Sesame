...
class KMeans : public SESAME::OfflineClustering {
 public:
  ...
  void adjustClusteringCenters(std::vector<PointPtr> &centers,std::vector<vector<PointPtr>> &groups);
  void refreshGroup(std::vector<std::vector<PointPtr>> &oldGroups, vector<vector<PointPtr>> &newGroups);
  void checkStopStatus(bool &flag,std::vector<std::vector<PointPtr>> &oldGroups,std::vector<std::vector<PointPtr>> &newGroups);
  void storeResult(std::vector<std::vector<PointPtr>> &groups, std::vector<PointPtr> &output);
  void runKMeans(int numberOfCenters, int numberOfInput, std::vector<PointPtr> &input, std::vector<PointPtr> &output, bool KMeansPP);
};
}
