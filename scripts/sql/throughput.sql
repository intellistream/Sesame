...
LEFT JOIN (SELECT algo, qps FROM main WHERE workload="KDD99_F") as kdd ON t.algo=kdd.algo
...
LEFT JOIN (SELECT algo, qps FROM main WHERE workload="ODS" AND num_points=100000 AND arr_rate=0) as ods ON t.algo=ods.algo
;
