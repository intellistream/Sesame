...
def draw_line(legend_name, data, colors, ods, throughput, purity, window, id):
    ...
    if throughput:
        ...
        file_name = "Window_"
        plt.ylabel('Cluster Number(x' + '$ \mathit{10}^{1}$' + ')', fontsize=30)
        data = np.log(data)
        legend_position = (0.45, 1.26)
    else:
        file_name = "CMM_"
...
def draw_all_pictures(purity, throughput, cmm, offline,
    ...
    compare_name = [["CFT", "MCs", "CoreT", "Grids", "DPT", "AMS"],
                    ...
                    ["NoOutlierD", "OutlierD-B", "OutlierD-T", "OutlierD", "OutlierD-BT"],
    ...
    if throughput_eds != '':
        ...
        draw_line(compare_name[1], purity_window_ods, colors[1], True, False, True, True, 0)
        ...
        # Throughput of Outlier Detection on EDS
        for i in range(len(compare_order[2])):
            purity_outlier_ods.append(purity_reader.iloc[compare_order[2][i] + 7].to_list()[1:])
        draw_line(compare_name[2], purity_outlier_ods, colors[2], True, False, True, False, 0)
